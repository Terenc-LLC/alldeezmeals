...
- **`detectDietaryTerms(note: string): string[]`**: top-level pure function in `src/App.tsx`. Lowercases the note; uses word-boundary regex to detect avoid-context phrasings ("no X", "X free", "X-free", "without X", "free of X") for these canonical terms: `nuts`, `peanuts`, `dairy`, `eggs`, `gluten`, `soy`, `shellfish`, `fish`, `sesame`. Trigger words: nuts ← nut/nuts/tree nut/treenut; fish ← fish only (naturally guarded by word boundaries — "shellfish" does not trigger "fish"). Returns deduped array in stable declaration order; returns `[]` when nothing matches.
